...
export class MoviesAPI {
  ...
  // Запит пошуку фільмів за кількома параметрами: пошуковим запитом (query), роком (year), жанром (genre) і регіоном (region):
  async getSearchMoviesAddParams(page, query, year, genre, region) {
    return axios.get(
      `${this.#BASE_URL}/3/search/movie?api_key=${this.#API_KEY}`,
      {
        params: {
